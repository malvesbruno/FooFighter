...
class Winner_Screen:
    def __init__(self, player):
        #setup geral
        self.item_list = []
        self.bg = pygame.image.load('graphics/pause/heaven.png').convert_alpha()
        ...
        self.option = ['press R to play again']
    ...
    def display(self, n_death):
        ...
        if self.choice:
            ...
            pygame.draw.rect(self.display_surface, '#6495ED', text_surf_rect, 0, 3)

            # self.display_surface.blit(text_surf, text_surf_rect)

            for index, item in enumerate(self.item_list):
                item.display(self.display_surface, self.selection_index, self.option[index], text_surf_rect, n_death),
...
class Item:
    ...
    def display(self, surface, selection_num, name, rect, n_death):
        self.num = 0
        ...
        if self.index == selection_num:
            pygame.draw.rect(surface, '#6495ED', rect_use)
        ...
        if self.frase_select:
            text = self.game_font.render(self.frase, False, "#FFFFFF")
            text_rect = text.get_rect(center=pos)
            rect_bg = text_rect
            rect_bg.center = text_rect.center
            rect_bg = rect_bg.inflate(10, 20)
            pygame.draw.rect(surface, '#6495ED', rect_bg, 1, 3)
